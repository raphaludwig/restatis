  ...
  if (response_type == "application/json") {
    ...
    if (response_parsed$Status$Code == 98) {
      error_message <- paste0(
        "You have requested a table too big for simple download. \n",
      ...
      message <- paste0(
        "You have successfully created a job with \n",
      ...
      stop(
        paste0(
          "There has been an error with your request (API error code: '",
...
logincheck_http_error <- function(database,
  ...
  if (length(database) == 1 && database != "all") {
    ...
    response <- .gen_api_core(
      endpoint = "helloworld/logincheck",
      database = database,
      ...
    )
    ...
    if ("genesis" %in% database) {
      logincheck_stop_or_warn(
        response = .gen_api_core(endpoint = "helloworld/logincheck", database = "genesis", ...),
        error = FALSE,
        verbose = verbose,
        database = "genesis"
      )
    ...
    if ("zensus" %in% database) {
      logincheck_stop_or_warn(
        response = .gen_api_core(endpoint = "helloworld/logincheck", database = "zensus", ...),
        error = FALSE,
        verbose = verbose,
        database = "zensus"
      )
    ...
    if ("regio" %in% database) {
      logincheck_stop_or_warn(
        response = .gen_api_core(endpoint = "helloworld/logincheck", database = "regio", ...),
        error = FALSE,
        verbose = verbose,
        database = "regio"
      )
...
set_credentials_auth <- function(path,
  ...
  if (isTRUE(use_token)) {
    ...
    username <- aux$login_destatis$username
    password <- aux$login_destatis$password
  }
  ...
  message(
    paste0("Your credentials for the ", ui_menu_database, " database have been saved to "),
    auth_path,
    ".",
    "\n\n",
